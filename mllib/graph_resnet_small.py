...
def build_resnet_small(h, w, channels, classes):
    ...
    activation = n_resnet_blocks("resnet1_", activation, 2, 16, 16)  # 2 blocks, 16 channels
    activation = n_resnet_blocks("resnet2_", activation, 2, 16, 32,
                                 downsample=True)  # 2 blocks, change from 16 channels to 32 channels, feature map to 16 high x 16 wide
    activation = n_resnet_blocks("resnet3_", activation, 2, 32, 64,
                                 downsample=True)  # 2 blocks, change from 32 channels to 64 channels, feature map to 8 high x 8 wide
